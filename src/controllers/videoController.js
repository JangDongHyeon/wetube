...
export const home = async (req, res) => {
  ...
  try {
    ...
    const videos = await Video.find({}).sort({ _id: -1 }); // sort 정령 -1 위 아래
...
export const postUpload = async (req, res) => {
  const {
    ...
    file: { path }
    // file:{location} S3을 사용할경유 주소받음
  } = req;
  const newVideo = await Video.create({
    fileUrl: path, //location은 s3사용시
    title,
    description,
...
export const postEditVideo = async (req, res) => {
  ...
  try {
    await Video.findOneAndUpdate({ _id: id }, { title, description }); // _id:id
